...
class User
{
    ...
    {
        $this->db->query('INSERT INTO users (usersName, usersEmail, usersUid, usersPwd, confirmToken) 
    ...
    {
        $this->db->query('SELECT * FROM users WHERE confirmToken = :token');
        ...
        //Check row
        if ($this->db->rowCount() > 0) {
            $this->db->query('UPDATE users SET confirmToken = "confirmed" WHERE confirmToken = :token');
            $this->db->bind(':token', $token);
